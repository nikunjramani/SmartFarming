...
for Fid in range(len(filepath)):	
	...
	cv2.drawContours(neworiginal,contours,-1,(0,0,255))
	...
	imghls = cv2.cvtColor(neworiginal, cv2.COLOR_BGR2HLS)
	...
	mask = cv2.bitwise_and(neworiginal,neworiginal,mask = thresh)
	...
	Infarea = 0
	for x in range(len(contours)):
		cv2.drawContours(img3,contours[x],-1,(0,0,255))
		cv2.imshow('Contour masked',img3)

		#Calculating area of infected region
		Infarea += cv2.contourArea(contours[x])

	if Infarea > Tarea:
		Tarea = neworiginal.shape[0]*neworiginal.shape[1]
